...
func Init(ctx context.Context, c Config) (*mongo.Client, error) {
	...
	once.Do(func() {
		...
		log.Info(ctx).Str("address", cfg.Address).Str("cost", time.Since(start).String()).
			Msg("success to connect to MongoDB")
	})
...
func HandleDBError(e error) error {
	if e != nil {
		if e == mongo.ErrNoDocuments {
			return api.ErrResourceNotFound
		}
		return api.ErrInternal.WithError(e)
	}
	return nil
}
