...
var (
	...
	ErrParseFilting            = newErrorMessage(http.StatusBadRequest, 4104, "failed to parse filting parameters")
	ErrParsePaging             = newErrorMessage(http.StatusBadRequest, 4105, "failed to parse paging parameters")
	ErrParseRange              = newErrorMessage(http.StatusBadRequest, 4106, "failed to parse range parameters")
	ErrInvalidMissingParameter = newErrorMessage(http.StatusBadRequest, 4107, "missing parameters")
	ErrPageArgumentsTooLarge   = newErrorMessage(http.StatusBadRequest, 4108, "page arguments too large")
	ErrInvalidResourceStatus   = newErrorMessage(http.StatusBadRequest, 4109, "invalid resource status")
	ErrFailedExchange          = newErrorMessage(http.StatusBadRequest, 4110, "failed to exchange an authorization code for a token")
	ErrShopifyNeedShopName     = newErrorMessage(http.StatusBadRequest, 4111, "shopify integration need a shop name")
	ErrInvalidID               = newErrorMessage(http.StatusBadRequest, 4112, "invalid mongodb id")
	ErrProcessIsNil            = newErrorMessage(http.StatusBadRequest, 4113, "process is nil")
	ErrQuotaLimit              = newErrorMessage(http.StatusBadRequest, 4114, "too many resource, quota limit")
	ErrInvalidParameter        = newErrorMessage(http.StatusBadRequest, 4115, "invalid parameters")
	ErrUnsupportedCurrency     = newErrorMessage(http.StatusBadRequest, 4116, "unsupported currency")
	ErrUnsupportedMethod       = newErrorMessage(http.StatusBadRequest, 4117, "unsupported method")
	ErrVerificationFailed      = newErrorMessage(http.StatusBadRequest, 4118, "verification failed")
	ErrUnsupportedKind         = newErrorMessage(http.StatusBadRequest, 4119, "unsupported kind")
